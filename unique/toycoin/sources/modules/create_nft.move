...
module toycoin::create_nft {
    ...
    public entry fun delayed_mint_event_ticket(module_owner: &signer, receiver: &signer) acquires ModuleData {
        ...
        assert!(signer::address_of(module_owner) == @toycoin, error::permission_denied(ENOT_AUTHORIZED));

        // Mint token to the receiver.
        let module_data = borrow_global_mut<ModuleData>(@toycoin);
